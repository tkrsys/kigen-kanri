...
function GanttChart({clients,onEditClient}){
  ...
  const renderBarContent=(client,dt,rowH,barH)=>{
    ...
    if(status==='expired'){return<><div onClick={e=>{e.stopPropagation();onEditClient(client);}} title={tip} style={{position:'absolute',left:Math.max(0,endX-2),top:(rowH-barH)/2,width:6,height:barH,borderRadius:2,background:'#c0392b',cursor:'pointer',zIndex:1}}/><span style={{position:'absolute',left:Math.max(0,endX+8),top:(rowH-10)/2,fontSize:8,color:'#c0392b',fontWeight:600,whiteSpace:'nowrap'}}>{lbl} 超過</span></>;}
    const bw=Math.max(3,endX-barStartX);
    return<><div onClick={e=>{e.stopPropagation();onEditClient(client);}} title={tip} style={{position:'absolute',left:barStartX,top:(rowH-barH)/2,width:bw,height:barH,borderRadius:3,background:barColor,opacity:0.8,cursor:'pointer'}} onMouseEnter={e=>e.currentTarget.style.opacity='1'} onMouseLeave={e=>e.currentTarget.style.opacity='0.8'}/>{endX>16&&endX<chartW-24&&<span style={{position:'absolute',left:endX+3,top:(rowH-10)/2,fontSize:8,color:lblColor,fontWeight:600,whiteSpace:'nowrap'}}>{lbl}</span>}</>;
  };

  /* ★折りたたみ時の重ねバー描画（認定→長期→短期の順で重ね、短期が一番手前） */
  const renderStackedBars=(client)=>{
    const h=STACK_H;const layers=[];
    /* 下から順に描画: 認定(紺)→長期(緑)→短期(黄土) */
    STACKED_ORDER.forEach((dt,li)=>{
      ...
      const barStartX=Math.max(0,Math.min(chartW,todayX));
      const barColor=GANTT_BAR_COLORS[dt.key].bar;
      const barH=h-li*2; /* 認定=18, 長期=16, 短期=14 */
      ...
      if(status==='expired'){
        ...
        layers.push(<span key={dt.key+'l'} style={{position:'absolute',left:Math.max(0,endX+8),top:topOff,fontSize:7,color:'#c0392b',fontWeight:600,whiteSpace:'nowrap',zIndex:10+li}}>{lbl}</span>);
      } else {
        const bw=Math.max(3,endX-barStartX);
        layers.push(<div key={dt.key+'b'} title={tip} style={{position:'absolute',left:barStartX,top:topOff,width:bw,height:barH,borderRadius:3,background:barColor,opacity:0.85,zIndex:10+li}}/>);
        if(endX>16&&endX<chartW-24){
          layers.push(<span key={dt.key+'l'} style={{position:'absolute',left:endX+3,top:topOff,fontSize:7,color:barColor,fontWeight:600,whiteSpace:'nowrap',zIndex:10+li}}>{lbl}</span>);
        }
      }
    });
    return layers;
  ...
  /* 月境界のグリッド線を描画 */
  const renderGridLines=(rowH)=>months.map((_,i)=>{if(i===0)return null;const m=months[i];const isJan=m.getMonth()===0;return<div key={'g'+i} style={{position:'absolute',top:0,bottom:0,left:MON_W*i,width:isJan?2:1,background:isJan?'#c8c8c0':'#f0efe8',pointerEvents:'none'}}/>;});
  const renderTodayLine=(rowH)=><div style={{position:'absolute',top:0,bottom:0,left:Math.max(0,todayX),width:2,background:'#c0392b',opacity:0.4,pointerEvents:'none'}}/>;
              ...
              if(!isExp){
                  ...
                  <tr key={`c-${client.id}`} onClick={()=>toggleExpand(client.id)} style={{cursor:'pointer',borderTop:ci>0?'1px solid #d8d8d0':'none'}}>
                    <td style={{position:'sticky',left:0,zIndex:3,background:'#fafaf8',borderRight:'2px solid #d8d8d0',borderLeft:`3px solid ${wc}`,padding:'3px 5px',verticalAlign:'middle'}}>
                      <div style={{display:'flex',alignItems:'center',gap:4}}>
                        <span style={{fontSize:8,color:'#8888a0',flexShrink:0,transform:'rotate(0deg)',transition:'transform 0.2s'}}>▶</span>
                    ...
                    <td colSpan={MONTHS} style={{padding:0,height:STACK_H+6,borderBottom:'none'}}>
                      <div style={{width:chartW,height:STACK_H+6,position:'relative',paddingTop:3}}>
                        ...
                        {renderGridLines(STACK_H+6)}
                        {renderTodayLine(STACK_H+6)}
                      </div>
                    </td>
                  </tr>
                );
                ...
                rows.push(
                  <tr key={`n-${client.id}`} onClick={()=>toggleExpand(client.id)} style={{cursor:'pointer',borderTop:ci>0?'1px solid #d8d8d0':'none'}}>
                    <td style={{position:'sticky',left:0,zIndex:3,background:'#fafaf8',borderRight:'2px solid #d8d8d0',borderLeft:`3px solid ${wc}`,padding:'3px 5px',verticalAlign:'middle'}}>
                      <div style={{display:'flex',alignItems:'center',gap:4}}>
                        <span style={{fontSize:8,color:'#8888a0',flexShrink:0,transform:'rotate(90deg)',transition:'transform 0.2s'}}>▶</span>
                        <div style={{minWidth:0}}>
                          <div style={{fontSize:10,fontWeight:600,color:'#1a1a2e',lineHeight:1.2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:NAME_W-30}}>{client.name}</div>
                          <div style={{fontSize:7,color:'#8888a0'}}>{client.care_manager||''}</div>
                        </div>
                  ...
                  rows.push(
                    <tr key={`b-${client.id}-${dt.key}`}>
                      <td style={{position:'sticky',left:0,zIndex:3,background:'#fafaf8',borderRight:'2px solid #d8d8d0',padding:'0 5px 0 18px',height:ROW_H,borderBottom:isLast?'1px solid #eceae3':'none',verticalAlign:'middle'}}>
                        <span style={{fontSize:8,color:'#8888a0'}}>{dt.short}</span>
                      </td>
...
function RegisterScreen({pin,onBack,onRegistered,managers:managerList,gearMenu,isAdmin}){
  ...
  const[name,setName]=useState('');const[careManager,setCareManager]=useState(initialCM);const[ninteiEnd,setNinteiEnd]=useState('');const[longEnd,setLongEnd]=useState('');const[shortEnd,setShortEnd]=useState('');const[saving,setSaving]=useState(false);const[error,setError]=useState('');const[success,setSuccess]=useState('');
  ...
  const inputStyle={width:'100%',padding:10,fontSize:14,border:'1px solid #d8d8d0',borderRadius:6,outline:'none',boxSizing:'border-box',color:T.text};const labelStyle={display:'block',fontSize:12,fontWeight:500,color:T.sub,marginBottom:6};
...
export default function KigenKanri(){
  ...
  useEffect(()=>{const saved=localStorage.getItem('kigen-pin');if(saved)setPin(saved);else setLoading(false);const sm=localStorage.getItem('kigen-manager-filter');if(sm)setManagerFilter(sm);const sr=localStorage.getItem('auth_role');setIsAdmin(sr==='admin');},[]);
  ...
  const handleManagerFilterChange=v=>{setManagerFilter(v);localStorage.setItem('kigen-manager-filter',v);};
  const fetchClients=useCallback(async p=>{setLoading(true);try{const res=await fetch('/api/clients',{headers:{'x-pin':p}});if(res.ok){const data=await res.json();setClients(data.clients||[]);const sm={};(data.clients||[]).forEach(c=>{if(c.care_manager)sm[c.care_manager]=!!c.calendar_sync;});setCalSyncMap(sm);if(data.role)localStorage.setItem('auth_role',data.role);setIsAdmin(data.role==='admin');}else if(res.status===401){localStorage.removeItem('kigen-pin');localStorage.removeItem('auth_role');setPin(null);}}catch(e){console.error(e);}setLoading(false);},[]);
  ...
  const handleCalSyncToggle=async name=>{const nv=!calSyncMap[name];setCalSyncMap(p=>({...p,[name]:nv}));if(!nv)setCopiedManager(null);try{await fetch('/api/care-managers',{method:'PUT',headers:{'Content-Type':'application/json','x-pin':pin},body:JSON.stringify({manager_name:name,calendar_sync:nv})});}catch(e){console.error(e);}};
  const handleCopyUrl=async mn=>{const url=getCalendarFeedUrl(mn);try{await navigator.clipboard.writeText(url);setCopiedManager(mn);setTimeout(()=>setCopiedManager(p=>p===mn?null:p),2000);}catch{const ta=document.createElement('textarea');ta.value=url;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);setCopiedManager(mn);setTimeout(()=>setCopiedManager(p=>p===mn?null:p),2000);}};
  const managers=useMemo(()=>{const s=new Set(clients.map(c=>c.care_manager).filter(Boolean));return Array.from(s).sort();},[clients]);
  const filteredClients=useMemo(()=>{let l=clients;if(managerFilter!=='all')l=l.filter(c=>c.care_manager===managerFilter);return l;},[clients,managerFilter]);
  const summary=useMemo(()=>{const c={expired:0,warning:0,caution:0,safe:0,attention:0,unset:0};filteredClients.forEach(cl=>{if(!hasAnyDeadline(cl)){c.unset++;return;}const s=getClientStatuses(cl);if(s.has('expired'))c.expired++;if(s.has('warning'))c.warning++;if(s.has('caution'))c.caution++;if(s.has('safe'))c.safe++;if(s.has('expired')||s.has('warning')||s.has('caution'))c.attention++;});return c;},[filteredClients]);
  const filteredSortedClients=useMemo(()=>{const p={expired:0,warning:1,caution:2,safe:3};let l=filteredClients.map(c=>({...c,worstStatus:getWorstStatus(c)}));l=l.filter(c=>clientMatchesFilter(c,activeFilter));l.sort((a,b)=>(a.worstStatus===null?99:p[a.worstStatus]??5)-(b.worstStatus===null?99:p[b.worstStatus]??5));return l;},[filteredClients,activeFilter]);
  const handleSave=uc=>{setClients(p=>p.map(c=>c.id===uc.id?{...uc,calendar_sync:c.calendar_sync}:c));setEditClient(null);};
  const handleLogout=()=>{setShowGearMenu(false);if(!window.confirm('ログアウトしますか？'))return;localStorage.removeItem('kigen-pin');localStorage.removeItem('auth_role');localStorage.removeItem('portal_authed');localStorage.removeItem('auth_pin');setPin(null);setClients([]);setIsAdmin(false);};
  const handleAuth=(p,r)=>{setPin(p);setIsAdmin(r==='admin');};
  const handleRegistered=nc=>{setClients(p=>[...p,nc].sort((a,b)=>(a.name||'').localeCompare(b.name||'')));};
  ...
  if(mode==='calendarSync'){
    const csm=managerFilter!=='all'?managers.filter(m=>m===managerFilter):managers;
    return(<div style={{fontFamily:"'Noto Sans JP', sans-serif",background:T.bg,minHeight:'100vh',color:T.text}}><div style={{maxWidth:880,margin:'0 auto',padding:'24px 16px 100px'}}>
      ...
      <div style={T.card}><div style={T.secTitle}><span style={T.barStyle}></span>Googleカレンダー同期</div><p style={{margin:'0 0 16px',fontSize:12,color:T.muted,lineHeight:1.5}}>☑にすると、担当利用者の期限予定がGoogleカレンダーに自動同期されます。</p>
        <div style={{display:'flex',flexDirection:'column',gap:10}}>{csm.map(m=>{const sy=!!calSyncMap[m];const cp=copiedManager===m;return(<div key={m} style={{borderRadius:8,background:sy?'#e8f5f0':'#fafaf8',border:`1px solid ${sy?'#27766a':'#d8d8d0'}`,overflow:'hidden'}}><label style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',cursor:'pointer'}}><input type="checkbox" checked={sy} onChange={()=>handleCalSyncToggle(m)} style={{accentColor:'#2d5a7b',width:16,height:16,flexShrink:0}}/><span style={{fontSize:14,fontWeight:500,color:T.text}}>{m}</span><span style={{fontSize:11,color:sy?'#27766a':T.muted,marginLeft:'auto'}}>{sy?'同期ON':'同期OFF'}</span></label>{sy&&<div style={{padding:'0 14px 12px',borderTop:'1px solid #b8ddd0'}}><p style={{margin:'8px 0 6px',fontSize:11,color:T.sub,fontWeight:500}}>📋 カレンダー同期URL</p><div style={{display:'flex',gap:6,alignItems:'center'}}><input type="text" readOnly value={getCalendarFeedUrl(m)} style={{flex:1,padding:'7px 10px',fontSize:11,border:'1px solid #d8d8d0',borderRadius:5,background:'#fff',color:T.sub,outline:'none',boxSizing:'border-box',fontFamily:'monospace'}}/><button onClick={()=>handleCopyUrl(m)} style={{flexShrink:0,display:'flex',alignItems:'center',gap:4,padding:'6px 12px',fontSize:11,fontWeight:500,border:'1px solid #d8d8d0',borderRadius:5,cursor:'pointer',background:cp?'#27766a':'#fff',color:cp?'#fff':T.sub,transition:'all 0.2s'}}>{cp?<><CheckIcon/> コピー済</>:<><CopyIcon/> コピー</>}</button></div><p style={{margin:'6px 0 0',fontSize:10,color:T.muted,lineHeight:1.4}}>GoogleカレンダーのURL追加で登録してください</p></div>}</div>);})}</div>
      </div><div style={{fontSize:11,color:T.muted,textAlign:'center',padding:20}}>Copyright &copy; 2026 tkrsys All rights reserved.</div>
    </div></div>);
  }
