...
function getDaysUntil(dateStr) {
  if (!dateStr) return null; const today = new Date(); today.setHours(0,0,0,0);
  const normalized = typeof dateStr === 'string' ? dateStr.split('T')[0] : dateStr;
  return Math.floor((new Date(normalized + 'T00:00:00') - today) / (1000*60*60*24));
}
...
function buildCalendarTitles(typeKey, userName, dateStr) {
  if (!dateStr) return null; const normalized=typeof dateStr==='string'?dateStr.split('T')[0]:dateStr; const config=CAL_CONFIG[typeKey]; const d=new Date(normalized+'T00:00:00');
  const endLabel=`${d.getMonth()+1}/${d.getDate()}`;
  const pre=getNoticeDateStr(normalized,2),mid=getNoticeDateStr(normalized,1),day=getNoticeDateStr(normalized,0);
  const pD=new Date(pre+'T00:00:00'),mD=new Date(mid+'T00:00:00'),dD=new Date(day+'T00:00:00');
  return {
    pre:{date:`${pD.getMonth()+1}/${pD.getDate()}`,title:`【${config.label} 2ヶ月前】${userName} ${endLabel}(${getActionMonth(normalized,-1)}月 ${config.preAction})`},
    mid:{date:`${mD.getMonth()+1}/${mD.getDate()}`,title:`【${config.label} 1ヶ月前】${userName} ${endLabel}(${getActionMonth(normalized,0)}月 ${config.midAction})`},
...
const T = { bg:'#f5f3ee', card:{background:'#fff',border:'1px solid #d8d8d0',borderRadius:8,padding:'16px 20px',marginBottom:12,boxShadow:'0 1px 3px rgba(0,0,0,.06)'}, accent:'#2d5a7b', text:'#1a1a2e', sub:'#4a4a5a', muted:'#8888a0', border:'#d8d8d0',
  barStyle:{display:'inline-block',width:3,height:16,background:'#2d5a7b',borderRadius:2}, secTitle:{fontSize:14,fontWeight:600,color:'#2d5a7b',display:'flex',alignItems:'center',gap:8,marginBottom:12},
  ...
  return (
    <div style={{fontFamily:"'Noto Sans JP', sans-serif",background:T.bg,minHeight:'100vh',color:T.text}}>
      <div style={{maxWidth:880,margin:'0 auto',padding:'24px 16px 100px'}}>
        ...
        {/* Filters + Manager select in one row */}
