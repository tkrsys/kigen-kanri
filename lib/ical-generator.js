...
/** 当月25日 */
function getDateSameMonth(dateStr) {
  const d = new Date(dateStr + 'T00:00:00');
...
function getActionMonth(dateStr, offset) {
  const d = new Date(dateStr + 'T00:00:00');
  let m = d.getMonth() + 1 + offset; // 1-indexed
  if (m <= 0) m += 12;
  if (m > 12) m -= 12;
  return m;
}
...
function generateAllEventsInternal(client) {
  ...
  for (const type of typesToOutput) {
    ...
    // 当月 (day)
    const dayDate = getDateSameMonth(dateStr);
    events.push({
      type, timing: 'day', date: dayDate,
      title: `【${config.label}　　　　】${name} ${endLabel}`,
    });
  }
